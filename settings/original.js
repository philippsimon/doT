...
var tags = {
  interpolate: {
    ...
    func: function(m, code) {
      ...
      var cse = settings.append ?
        settings.startend.append : settings.startend.split;
  ...
  iterate: {
    ...
    func: function(m, iterate, iname, connector, vname) {
      ...
      return '\'; var arr' + sid + ' = ' + iterate + '; if( arr' + sid +
        ' ) { var ' + vname + ', ' + iname + ' = -1, l' + sid + ' = arr' +
        sid + '.length-1; while( ' + iname + ' < l' + sid + ' ){ ' + vname +
        ' = arr' + sid + '[' + iname + ' += 1]; out += \'';
    }
  },
  ...
  content_for: {
    ...
    func: function(m, id, compileParams) {
      ...
      if (id) {
        return '\'; contents[current_out] = out; ' +
          'out_stack.push(current_out); current_out=\'' +
...
function resolveDefs(block, compileParams) {
  ...
  return block.toString().replace(resolveDefs.define || re_skip, function(
    m,
    code,
    assign,
    value
    ...
    if (!(code in def)) {
      if (assign === ':') {
        if (that.defineParams) {
          value.replace(that.defineParams, function(
            m,
            param,
            v
          ) {
...
function execute_tags(str, compileParams) {
  ...
  var taglist = Object.keys(settings.tags).sort();

  for (var t_id in taglist) {
    var t_name = taglist[t_id];
    var tag = settings.tags[t_name];
...
function function_basics(str, compileParams) {
  if (compileParams.multiple_contents) {
    return 'var out_stack = [], contents = {}, current_out = \'_content\'; ' +
      'var out = \'' + str +
      '\'; contents[current_out] = out; return contents;';
  } else {
...
function functionize(str, compileParams) {
  ...
  if (!settings.selfcontained) {
    if (global && !('encodeHTML' in global)) {
      global.encodeHTML = compileParams.doT.encodeHTMLSource(
        settings.doNotSkipEncoded);
    }
  } else {
    str = 'var encodeHTML = (' +
      compileParams.doT.encodeHTMLSource.toString() +
  ...
  try {
    ...
    throw new Error(e + ' in `new Function \'' + settings.varname +
      '\', "' + str + '"`');
  }
}
