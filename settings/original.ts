...
var tags = {
  interpolate: {
    ...
    func: function(m, code) {
      var settings: DotCore.settings = this;
  ...
  iterate: {
    ...
    func: function(m: string, iterate: string, iname: string, vname: string) {
      if (!iterate) {
        return "'; } } out += '";
      }
      if (!vname) {
  ...
  content_for: {
    regex: /\{\{>([\s\S]*?)\}\}/g,
    func: function(m: string, id: string, compileParams: DotCore.compile_args.compileParams) {
  ...
  xx_includeDynamic: {
    regex: /\{\{@@\s*(\S+?)\(([\s\S]*?)\)\s*\}\}/g,
    func: function(m: string, tmpl: string, args: any) {
      var settings: DotCore.settings = this;
  ...
  zz_evaluate: {
    regex: /\{\{([\s\S]*?)\}\}/g,
    func: function(m: string, code: string) {
...
class resolveDefs {
  ...
  static use = /\{\{#([\s\S]+?)\}\}/g;
  static define = /\{\{##\s*([\w\.$]+)\s*(\:|=)([\s\S]+?)#\}\}/g;

  public defineParams: RegExp;
  public useParams: RegExp;

  constructor (
    block: any,
    compileParams: DotCore.compile_args.compileParams
  ) {
